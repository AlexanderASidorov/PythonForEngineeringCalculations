...
# 2. Имеет множество уже написанных и свободно распростроняемых библиотек, для решения тех или иных задач;
# 3. Имеет множество документации по работе с данными и решения инженерных задач;
# 4. В отличии, например, от MatLab полностью бесплатный со множеством свободно-распространяемых сред разработки
#
#
#
# В рамках нашего курса мы будем изучать только то, что необходимо нам для решения конкретных инженерных задач:
# расчет механических свойств материала, апроксимация данных по сопротивлению деформации и т.д.
# Более глубоко вы можете изучить Python по специализированной литературе
#        например Python для сложных задач. Наука о данных и машинное обучение. Автор Дж. Вандер Плас
...
# ну и не забываем про официальные tutorials docs.python.org и tutorials основных библиотек, с которыми мы будем работать:
    ...
    # Библиотека Matplotlib (для построения графиков и прочей визуализации данных) https://matplotlib.org
#
#
#
# Python - язык императивного программирования, т.е. вы фактически пишите компьютеру ряд инструкций (команд),
#  которые он последовательно выполняет.
# Программа на языке Python фактически представляет собой скрипт (текстовый файл) с расширением .py
#
#
...
# Она хранит в себе все математические функции, которыми мы пользуемся в инженерных расчетах.
# Если мы в данные момент, не импортировав эту библиотеку в консоле попробуем задать 
# команду cos(b), то выскочит ошибка # NameError: name 'cos' is not defined
# Если мы подключим эту библиотека с помощью команды import math, то нам станет 
# доступна, в том числе, функция определения cos перременной
import math
