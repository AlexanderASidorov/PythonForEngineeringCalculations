...
A=[100, 110, 120, 130, 140] # площади поперечного сечения колонны, мм**2
...
K=np.delete(K, Length, axis=0) # удаляем последнюю сторку
K=np.delete(K, Length, axis=1) # удяляем последний столбец
R=np.delete(R, Length)
