...
# то же самое, но с четким указанием типов входных переменных
def root_of_degree (a: (int), b: (int), c: (int), d: int) -> float:
    '''
    Кубический корень из произведения трех чисел
...
# Пример простейшей функции, но с параметрами по-умолчанию и проверкой 
# типов входных переменных
...
def calculate_sum_of_3_numbers_and_power (a, b, c, d=1/3):
    ...
    list_of_arguments = [a, b, c, d]
    
    for item in list_of_arguments:
        if isinstance(item, (float, int)):
            pass
        else:
            raise ValueError ('Все аргументы этой функции должны быть типа int или float')
...
def high_order_function (x, function, *args):
    
    return function(x, *args)



def polynome_2 (x, *args):
    ...
    f = coefs[0] + coefs[1]*x + coefs[2]*x**2
    
    return f, coefs



def polynome_3 (x, *args):
    
    args = list(args)
    
    coefs = args[0:4]
    
    f = coefs[0] + coefs[1]*x + coefs[2]*x**2 + coefs[3]*x**3
    return f, coefs


def polynome_4 (x, *args):
    
    args = list(args)
    
    coefs = args[0:5]
    
    f = coefs[0] + coefs[1]*x + coefs[2]*x**2 + coefs[3]*x**3 + coefs[4]*x**4
    return f, coefs

coefs= [2, 4, 5, 4, 5]


high_order_function_polynome_2, _  = high_order_function (2, polynome_2, *coefs)
