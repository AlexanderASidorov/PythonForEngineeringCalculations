...
#%%
'''
Чуть позже мы будем проходить функции в Python и там нам необходимо бывает
проверить тип переменной, которую мы подаем в функцию. Для этого мы будем использовать
следующие конструкции на базе оператора if
Вернемся к задаче нахождения корней уравнения, но запишем код в немного другом виде
'''
...
# проверяем, что k - переменная типа float или integer
if isinstance(k, (float, int)) and isinstance(b, (float, int)):
    pass
else:
    raise TypeError ('Переменные k и b должны быть типа float или integer')

# проверяем, что уравение имеет одно решение:
if k==0 and b==0:
    raise ValueError ('Если оба коэффициента равны 0, то уравнение имеет бесконечное количество корней')

elif k==0:
    raise ValueError ('Если k = 0, то уравнение коренй не имеет')
